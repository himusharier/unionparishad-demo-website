...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if(isset($_POST['perPageLimit'])){
        ...
        if (clean_inputs($_POST['perPageLimit']) == "100") {
            ...
            echo "<script type='text/javascript'> document.location = 'admin/disability-list'; </script>";
        }
...
<script>
    document.title = 'প্রতিবন্ধী তথ্য তালিকা - ইউনিয়ন পরিষদ';
</script>
...
<div class="content-body-main-div">
    ...
    <p class="page-direction"><i class="fa fa-link"></i> ড্যাশবোর্ড / প্রতিবন্ধী তথ্য তালিকা /</p>
    ...
    <div class="dashboard-table-container">
        ...
        $sqlr = "SELECT * FROM house_holding_database WHERE disability='হ্যাঁ' ORDER BY id ASC LIMIT {$start_from}, ".$results_per_page;
        $resultr = mysqli_query($db, $sqlr);
        $countr = mysqli_num_rows($resultr);

        $sqlr2Count = "SELECT * FROM house_holding_database WHERE disability='হ্যাঁ'";
        $resultr2Count = mysqli_query($db, $sqlr2Count);
        $countr2Count = mysqli_num_rows($resultr2Count);
        ?>
        <p class="dashboard-table-container-heading">প্রতিবন্ধী তথ্য তালিকা (<?php echo banglaNumber($countr2Count); ?> জন)</p>
            ...
            <table class="responstable">
                ...
                ?>
                    <tr title="Entry By: <?php echo $dataEntryRow['full_name']; ?>">
                        ...
                        <td><?php echo banglaNumber($rowr['personName']) ?></td>
    ...
    <div class="bottom-pagination">
        <?php
        $sqlpn = "SELECT COUNT(id) AS total FROM house_holding_database WHERE disability='হ্যাঁ' ORDER BY id ASC";
        ...
        for ($i=1; $i<=$total_pages; $i++) {
            ...
            echo "' href='admin/disability-list?page={$i}'>{$i}</a>";
