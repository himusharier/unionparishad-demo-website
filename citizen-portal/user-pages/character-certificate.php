...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['submit-btn'])) {
        ...
        if (empty($_POST['linkedForm']) || empty($_POST['fullName']) || empty($_POST['guardianName']) || empty($_POST['motherName']) || empty($_POST['nidNo']) || empty($_POST['birthDate']) || empty($_POST['village']) || empty($_POST['wardNo']) || empty($_POST['upozilla']) || empty($_POST['zilla']) || empty($_POST['applyType'])) {
            ...
            $pr1Sql = "INSERT INTO character_certificate_apply (linkedForm, certificateID, fullName, guardianName, motherName, nidNo, birthDate, village, wardNo, upozilla, zilla, regStatus, paymentStatus, applyDate, applyType, ipAddress, status, statusBy) VALUES ('$linkedForm', '$certificateID', '$fullName', '$guardianName', '$motherName', '$nidNo', '$birthDate', '$village', '$wardNo', '$upozilla', '$zilla', 'Done', 'Unpaid', '$applyDate', '$applyType', '$ipAddress', 'Requested', '')";

            if (mysqli_query($db, $pr1Sql)) {
                $_SESSION['linkedForm'] = $linkedForm;
                $_SESSION['certificateID'] = $certificateID;
                echo "<script type='text/javascript'> document.location = 'citizen/character-certificate-details'; </script>";
            }
...
<script>
    document.title = 'চারিত্রিক সনদ - ইউনিয়ন পরিষদ';
</script>
...
<div class="content-body-main-div">
    ...
    <p class="page-direction"><i class="fa fa-link"></i> নাগরিক পোর্টাল / চারিত্রিক সনদ /</p>

    <div class="admin-dashboard-content-div">

        <h2 style="font-family:'Bangla';color: #5e5e5e;border-bottom:1px solid #5e5e5e;">চারিত্রিক সনদ আবেদনপত্রঃ</h2>
        ...
        <div class="dashboard-table-container" style="margin-top: 15px;">
            <p class="dashboard-table-container-heading">চারিত্রিক সনদ তথ্য</p>
    ...
    <div>
        ...
        <table class="responstable">
            ...
            $sqlr = "SELECT * FROM character_certificate_apply WHERE linkedForm = '$formID' ORDER BY id ASC";
