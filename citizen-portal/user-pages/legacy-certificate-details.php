...
if (!empty($_SESSION['linkedForm']) && !empty($_SESSION['certificateID'])) {
    ...
    echo "<script type='text/javascript'> document.location = 'citizen/legacy-certificate'; </script>";
    die();
}
...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['payment-done'])) {
        ...
        $paymentUpdate = "UPDATE legacy_certificate_apply SET paymentStatus='Paid' WHERE (linkedForm='$linkedForm' AND certificateID='$certificateID')";
...
<div class="content-body-main-div">
    ...
    <p class="page-direction"><i class="fa fa-link"></i> নাগরিক পোর্টাল / ওয়ারিশনামা সনদ / বিস্তারিত /</p>

    <div class="admin-dashboard-content-div">

        <h2 style="font-family:'Bangla';color: #5e5e5e;border-bottom:1px solid #5e5e5e;">ওয়ারিশনামা সনদ বিস্তারিতঃ</h2>
        <?php
        $sqlr = "SELECT * FROM legacy_certificate_apply WHERE (linkedForm = '$linkedForm' AND certificateID = '$certificateID') ORDER BY id ASC";
        ...
        <table class="responstable">
            ...
            <tr>
                <td>সনদ তথ্য</td>
                <td>
                    <b>নামঃ</b> মৃত <?php echo $rowr['fullName'] ?><br/>
                    ...
                    <b>পৌরসভাঃ</b> <?php echo $rowr['pouroshova'] ?><br/>
                    <b>জেলাঃ</b> <?php echo $rowr['zilla'] ?><br/>
                    <br/>

                    <?php
                        $sqlPr = "SELECT * FROM legacy_certificate_apply_heredity WHERE (linkedForm = '$linkedForm' AND linkedCertificate = '$certificateID') ORDER BY id ASC";
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td><?php echo $rowPr['personName']; ?></td>
                            <td><?php echo $rowPr['relationType']; ?></td>
                        </tr>
                        <?php
                        }
                        }
    ...
    <div id="print-main-div">
        <div style="border: 3px solid #1e2039;padding: 10px;">
            <div class='print-header-top'>
                <p>স্মারক নংঃ ইউ/পরি/ওয়ারি/স/<?php echo banglaNumber($certificateID); ?></p>
            ...
            <p style="text-align: center;margin-top: 80px;color: #1e2039;font-size: 52px;font-weight: bold;">ওয়ারিশনামা সনদপত্র</p>
            <p style="font-size: 24px;margin-top: 50px;padding: 0 80px;">
                এই মর্মে প্রত্যয়ন করা যাচ্ছে যে,
                নামঃ <b style="display: inline-block;padding: 0 10px;">মৃত <?php echo $rowr['fullName'] ?></b>
                ...
                পৌরসভাঃ <b style="display: inline-block;padding: 0 10px;"><?php echo $rowr['pouroshova'] ?></b>
            ...
            <div class="certificateTable">
                <table>
                    ...
                    while ($rowPr = mysqli_fetch_assoc($resultPr)) {
                        ?>
                        <tr>
                            <td><?php echo banglaNumber(@$snr2 += 1); ?></td>
                            <td><?php echo $rowPr['personName']; ?></td>
                            <td><?php echo $rowPr['relationType']; ?></td>
                        </tr>
                        <?php
                ...
                <br/><br/><br/><br/>
            <p style="font-size: 24px;padding: 0 80px;">
                <a style="text-align: center;display: inline-block;">আমি তার বিদেহী আত্মার মাগফিরাত কামনা করি।</a>
            </p>
            <div style="display: flex; justify-content: space-between;margin-top: 100px;margin-bottom: 30px;">
                <p></p>
